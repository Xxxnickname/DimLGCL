...
from rgcn.dsf import DSF_embedding
...
class RecurrentRGCN(nn.Module):
    ...
    def predict(self,que_pair, sub_graph,T_id, test_graph, num_rels, static_graph, test_triplets,time_point, use_cuda):
        with torch.no_grad():
    ...
    def get_loss(self, que_pair, sub_graph,T_idx, glist, triples, static_graph, time_point, d_dict_er2e,use_cuda):
        ...
        if self.p.use_vocab:
            ...
            for i in range(s_len):
                distribute_t_er2e = torch.zeros(self.sequence_len, self.num_ents).cuda()
                
                if self.p.use_his==False:
                    distribute_er2e = None
                    break
                
                if r[i].item() >= self.num_rels:
                    row, column = d_dict_er2e[(s[i].item(), r[i].item() - self.num_rels)]
                else:
            ...
            loss_vocal += self.vocabulary(s, r, o, distribute_er2e)


        return loss_ent, loss_rel, loss_static, loss_cl, loss_vocal
